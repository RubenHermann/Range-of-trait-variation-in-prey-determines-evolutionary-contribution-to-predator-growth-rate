...
Geber_method <- function(dat,t){
  ...
  for(j in 1:length(t)){
    a=sto[1]+sto[2]*dat$freq_def[j]*dat$C_all[j]
    
    b=sto[1]+sto[2]*dat$freq_def[j]*dat$C_all[j+1]
      
    c=sto[1]+sto[2]*dat$freq_def[j+1]*dat$C_all[j]
      
    d=sto[1]+sto[2]*dat$freq_def[j+1]*dat$C_all[j+1]
      
    evo=((c-a)+(d-b))/2
    eco=((b-a)+(d-c))/2
    dat$evo[j] <- evo
    dat$eco[j] <- eco
    dat$eco_evo[j] <- eco/evo
    dat$ln_eco_evo <- log(abs(dat$eco_evo))
