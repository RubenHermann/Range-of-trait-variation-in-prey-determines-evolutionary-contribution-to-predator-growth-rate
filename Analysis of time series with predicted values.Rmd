...
#m2 <- glmer(algae_ml ~  cc*Freq.clone.1*day_fact+(1|Rep) ,family =Gamma(link = "log"),data=cr_data2)
#m_cr2 <- glmer(algae_ml ~  cc*day+Freq.clone.1+(1|Rep) ,family =Gamma(link = "log"),data2=cr_data2)
#anova(m_cr,m2,test="LRT")
summary(m_cr)
Anova(m_cr)

sum_cr <- capture.output(summary(m_cr))
an_cr <- capture.output(Anova(m_cr))
setwd("C:/Nextcloud/Work_data2/Assays/IER traits (Chapter 2)/Skripts/Testing analysis/Output")
cat("Algae time series summary_predicted",sum_cr,file="Algal analysis summuary.txt",sep ="\n", append=TRUE)
cat("Algae time series Anova_predicted",an_cr,file="Algal analysis Anova.txt",sep ="\n", append=TRUE)
```
...
write.csv(pre,"Oldvspredicted_data.csv")
```

Applying the geber method
```{r}
data$x_var = data$pred_freq_clone.1*data$algae_ml
...
for (i in 1:length(ALL)){
  for (j in 1:nrow(ALL_comp[[i]])){
    a=sto[[i]][1]+sto[[i]][2]*ALL_comp[[i]]$pred_freq_clone.1[j]*ALL_comp[[i]]$algae_ml[j]
    
    b=sto[[i]][1]+sto[[i]][2]*ALL_comp[[i]]$pred_freq_clone.1[j]*ALL_comp[[i]]$algae_ml[j+1]
    
    c=sto[[i]][1]+sto[[i]][2]*ALL_comp[[i]]$pred_freq_clone.1[j+1]*ALL_comp[[i]]$algae_ml[j]
    
    d=sto[[i]][1]+sto[[i]][2]*ALL_comp[[i]]$pred_freq_clone.1[j+1]*ALL_comp[[i]]$algae_ml[j+1]
...
p3<-ggplot(data=subset(CR1_6_mean,variable=="levo_eco"),aes(x=day,y=mean,color=fact_freq_def))+
  ...
  #geom_line(data=subset(CR1_6_mean_m,variable=="levo_eco"),aes(x=day,y=mean),size=3,color="black")+
   geom_hline(aes(yintercept=0),size=1,color="black")+
  #geom_line(data=CR1_6,aes(y=evo_eco,x=day,color=fact_freq_def,group=id),linetype="dashed")+
  #geom_point(data=CR1_6,aes(y=evo_eco,x=day,color=fact_freq_def,group=id))+
  ggtitle("CR1 with CR6 ")+
  ylab("Log(|Evolution/Evcology|)")+xlab("Day")+
  ylim(c(-4,4))+
  #scale_fill_manual('Starting frequency of CR1')+
...
p3_2<-ggplot(subset(CR1_7_mean,variable=="levo_eco"),aes(x=day,y=mean,color=fact_freq_def))+
  geom_line(size=3,position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4))+
  theme_classic()+
  xlim(c(-0.3,8.3))+
  geom_point(size=4,position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4))+
  geom_errorbar(aes(ymin=mean-se,ymax=mean+se),position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4),size=1,width=0.5)+
  #geom_line(data=subset(CR1_7_mean_m,variable=="levo_eco"),aes(x=day,y=mean),size=3,color="black")+
  geom_hline(aes(yintercept=0),size=1,color="black")+
  #geom_line(data=CR1_7,aes(y=evo_eco,x=day,color=fact_freq_def,group=id),linetype="dashed")+
  #geom_point(data=CR1_7,aes(y=evo_eco,x=day,color=fact_freq_def,group=id))+
...
p3_1<-ggplot(subset(CR2_6_mean,variable=="levo_eco"),aes(x=day,y=mean,color=fact_freq_def))+
  geom_line(size=3,position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4))+
  geom_point(size=4,position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4))+
  geom_errorbar(aes(ymin=mean-se,ymax=mean+se),position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4),size=1,width=0.5)+
  theme_classic()+
  #geom_line(data=CR2_6,aes(y=evo_eco,x=day,color=fact_freq_def,group=id),linetype="dashed")+
  #geom_point(data=CR2_6,aes(y=evo_eco,x=day,color=fact_freq_def,group=id))+
  #geom_line(data=subset(CR2_6_mean_m,variable=="levo_eco"),aes(x=day,y=mean),size=3,color="black")+
  geom_hline(aes(yintercept=0),size=1,color="black")+
...
p3_3<-ggplot(subset(CR2_7_mean,variable=="levo_eco"),aes(x=day,y=mean,color=fact_freq_def))+
  ...
  xlim(c(-0.3,8.3))+
  geom_errorbar(aes(ymin=mean-se,ymax=mean+se),position = position_jitterdodge(jitter.height=0,jitter.width=0.1,dodge.width = 0.4),size=1,width=0.5)+
  #geom_line(data=subset(CR2_7_mean_m,variable=="levo_eco"),aes(x=day,y=mean),size=3,color="black")+
  geom_hline(aes(yintercept=0),size=1,color="black")+
  #geom_line(data=CR2_7,aes(y=evo_eco,x=day,color=fact_freq_def,group=id),linetype="dashed")+
  #geom_point(data=CR2_7,aes(y=evo_eco,x=day,color=fact_freq_def,group=id))+
...
ggsave("../Graphs/Revision/Newlevo_eco_predicted_values.pdf",figure,width=35,height=35)
```

Loading data
```{r}
traits <- read.csv("../Toms Traitspace.csv")
geb <- data
...
ggsave("../Graphs/Revision/mean levo_eco for trait distance_pos_predicted values.pdf")
```
...
sum_levo <- capture.output(summary(m_levo))
an_levo <- capture.output(Anova(m_levo))
setwd("Analysis output/")
cat("Eco over Evo influence time series summary",sum_levo,file="Eco over Evo influence analysis summuary_new_pos_predicted.txt",sep ="\n", append=TRUE)
cat("Eco over Evo influence  time series Anova",an_levo,file="Eco over Evo influence analysis Anova_new_pos_predicted.txt",sep ="\n", append=TRUE)
